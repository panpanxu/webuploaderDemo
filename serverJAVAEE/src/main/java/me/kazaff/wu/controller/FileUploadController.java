...
public class FileUploadController {
    ...
    @ResponseBody
    public String fileUpload(FileInfo info, @RequestParam(value = "file", required = false) MultipartFile file) {
    ...
    @RequestMapping(value = "fileUpload", method = RequestMethod.POST, headers = ("content-type=application/*"))
    @ResponseBody
    public String fileUpload(String status, FileInfo info) {
        ...
        if (status.equals("md5Check")) {    //秒传验证
            ...
            return "{\"status\": 1, \"path\": \"" + path + "\", \"message\": \"分块合并完成\"}";
        }
